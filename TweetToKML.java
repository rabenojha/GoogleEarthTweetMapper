...
public class TweetToKML {
	...
	public static void to_KML(String working_directory) {
		...
		try {
			FileReader fileReader = new FileReader(working_directory + "\\tweet.csv"); // read CSV file from local
																						// directory
			...
			while (s.hasNextLine()) {
				...
				if (hashtag != null) {
					colours = "FFFF0000"; // blue // be careful with the color codes used here. KML uses an 8 digit HEX
											// color format rather than traditional HEX format.
				}
				if (hashtag == "") {
					colours = "FFFF00FF"; // pink // be careful with the color codes used here. KML uses an 8 digit HEX
											// color format rather than traditional HEX format.
				}
		...
		try {
			...
			filewriter.write(kml_header + ImagetoKML.image_to_kml(working_directory) + kmlbody + kml_footer); // also adds the returned string from ImagetoKML class
			filewriter.close();
